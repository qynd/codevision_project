...
class _AdminApprovalScreenState extends State<AdminApprovalScreen> {
  ...
  Future<void> _approveTask(String taskId) async {
    try {
      await supabase
          .from('tasks')
          .update({'status': 'Done', 'progress_percent': 100})
          .eq('id', taskId);
      setState(() {});
      if (mounted) {
        ScaffoldMessenger.of(
          context,
        ).showSnackBar(const SnackBar(content: Text("Tugas disetujui.")));
      }
    } catch (e) {
      if (mounted) {
        ScaffoldMessenger.of(
    ...
    try {
      ...
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(content: Text("Tugas ditolak (kembali ke Doing).")),
        );
      }
    } catch (e) {
      if (mounted) {
      ...
      body: FutureBuilder<List<Map<String, dynamic>>>(
        ...
        builder: (context, snapshot) {
          ...
          if (!snapshot.hasData || snapshot.data!.isEmpty) {
            return const Center(
              child: Text("Tidak ada tugas yang perlu disetujui."),
            );
          }
          ...
          return ListView.builder(
            ...
            itemBuilder: (context, index) {
              ...
              final projectName = task['projects'] != null
                  ...
                  : 'Unknown Project';
              final userName = task['users'] != null
                  ? task['users']['nama']
                  : 'Unknown User';
