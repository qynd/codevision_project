...
class _AdminProjectDetailScreenState extends State<AdminProjectDetailScreen>
  ...
  Future<List<Map<String, dynamic>>> _fetchTasks({
    bool onlyPendingApproval = false,
  }) async {
    try {
    ...
    try {
      ...
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(content: Text("Tugas berhasil disetujui (Done).")),
        );
      }
    } catch (e) {
      if (mounted) {
  ...
  Future<void> _rejectTask(String taskId) async {
    try {
      ...
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(
            content: Text("Tugas ditolak. Status kembali ke 'Doing'."),
          ),
        );
      }
    } catch (e) {
