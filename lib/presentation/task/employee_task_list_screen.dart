  ...
  Future<List<Map<String, dynamic>>> _fetchMyTasks() async {
    ...
    final response = await supabase
        ...
        .neq(
          'status',
          'Done',
        ) // Tampilkan yang belum selesai saja (atau urutkan status)
              ...
              content: Column(
                ...
                children: [
                  DropdownButton<String>(
                    ...
                    isExpanded: true,
                    items: ['To Do', 'Doing', 'Waiting Approval', 'Done'].map((
                      e,
                    ) {
                      bool enabled = true;
                      if (e == 'Done') {
                        enabled = false;
                      } // Pegawai tak bisa set Done langsung

                      return DropdownMenuItem(
                        value: e,
                        enabled: enabled,
                        child: Text(
                          e == 'Done' ? 'Done (Verified by Admin)' : e,
                          style: TextStyle(
      ...
      body: FutureBuilder<List<Map<String, dynamic>>>(
        ...
        builder: (context, snapshot) {
          ...
          if (!snapshot.hasData || snapshot.data!.isEmpty) {
            return const Center(child: Text("Hore! Tidak ada tugas pending."));
          }
          ...
          return ListView.builder(
            ...
            itemBuilder: (context, index) {
              ...
              final projectName = task['projects'] != null
                  ? task['projects']['nama_proyek']
                  : '-';
              final deadline = task['deadline'] != null
                  ? DateFormat('d MMM').format(DateTime.parse(task['deadline']))
                  : '-';
